...
//        specificlistId: specificlistId
        ...
        if ($request->isXMLHttpRequest()) {
            ...
            $query = $em->createQuery(
                "SELECT DISTINCT slv.specificlistvideoId, sl.specificlistId, u.userId, u.userName, 
                v.videoId, v.videoName, v.videoDescription, v.videoImage, v.videoContent, 
                v.videoUpdatedate, v.videoAmountViews, v.videoAmountComments, v.videoScore 
            ...
            $i = 0;
                ...
                while (isset($specificlistvideo[$i]['specificlistvideoId'])) {

                if (
                    file_exists("files/".$specificlistvideo[$i]['userId'].
                    "/".$specificlistvideo[$i]['videoId'].
                    "/".$specificlistvideo[$i]['videoImage'])
                    &&
                    file_exists("files/".$specificlistvideo[$i]['userId'].
                    "/".$specificlistvideo[$i]['videoId'].
                    "/".$specificlistvideo[$i]['videoContent'])
                )
                {
                    ...
                    $videoScore_Value = $specificlistvideo[$i]['videoScore'];
                } else
                {
